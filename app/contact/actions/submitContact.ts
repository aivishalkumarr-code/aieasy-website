...
export async function submitContact(
  ...
  if (!isSupabaseConfigured()) {
    return {
      success: false,
      message: "Form submission is currently unavailable. Please try again later or contact us directly.",
    };
  }
  ...
  // Format notes with all the additional fields
  const notes = `Service Interest: ${values.serviceInterest}
    ...
    if (resend) {
      try {
        ...
        await resend.emails.send({
          ...
          html: buildCustomerEmail(values.name, values.company, values.serviceInterest),
        });
