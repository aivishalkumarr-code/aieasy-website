...
export function BusinessTypePopup({ cards }: BusinessTypePopupProps) {
  ...
  const openModal = (businessType: string) => {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      formData.set("businessName", values.name.trim());
      formData.set("message", "");
      formData.set("businessType", selectedBusinessType);
      formData.set("source", "landing_page__business_type");
    ...
    <>
      ...
      {isOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/35 p-4 backdrop-blur-sm">
          <div className="relative w-full max-w-md rounded-2xl bg-white p-6 shadow-[0_24px_80px_rgba(15,23,42,0.25)] sm:p-7">
            <button
              type="button"
              onClick={closeModal}
            ...
            {!isSuccess ? (
              <>
                <h3 className="pr-8 text-2xl font-bold tracking-tight text-[#0F172A]">
                  Get a Free Website Plan for {selectedBusinessType}
                </h3>
                ...
                <form onSubmit={handleSubmit} className="mt-6 space-y-4">
                  <input type="hidden" name="source" value="landing_page__business_type" />
              ...
              <div className="py-4">
                ...
                <p className="mt-3 text-sm leading-6 text-[#475569]">
                  We&apos;ll contact you within 24 hours with a custom website plan for your {selectedBusinessType}.
                </p>
                <button
                  type="button"
                  onClick={closeModal}
                  className="mt-5 inline-flex h-11 w-full items-center justify-center rounded-xl bg-[#2563EB] px-4 text-sm font-semibold text-white transition hover:bg-[#1D4ED8]"
                >
                  Done
                </button>
              </div>
