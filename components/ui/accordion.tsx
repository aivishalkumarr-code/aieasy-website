...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between gap-4 py-5 text-left text-base font-semibold text-slate-900 transition hover:text-[#0D9488] [&[data-state=open]>svg]:rotate-180",
        className,
      )}
      {...props}
    >
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Content
    ...
    {...props}
  >
    <div className={cn("pb-5 pt-1 text-base leading-7", className)}>{children}</div>
  </AccordionPrimitive.Content>
));
